// frontend/src/repositories/mock/MockHydroHarvestResultRepository.ts
...
export class MockHydroHarvestResultRepository
  implements HarvestResultRepository
{
  ...
  async getAll(): Promise<HarvestResult[]> {
    if (!this.cache) {
      this.cache = await fetchMockData<HarvestResult[]>('hydro-harvests.json');
    }
    return this.cache!;
  }
