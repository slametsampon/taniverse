// frontend/src/repositories/mock/MockAquaHarvestResultRepository.ts
...
export class MockAquaHarvestResultRepository
  implements HarvestResultRepository
{
  ...
  async getAll(): Promise<HarvestResult[]> {
    if (!this.cache) {
      this.cache = await fetchMockData<HarvestResult[]>('aqua-harvests.json');
    }
    return this.cache!;
  }
