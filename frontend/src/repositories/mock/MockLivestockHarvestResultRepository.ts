// frontend/src/repositories/mock/MockLivestockHarvestResultRepository.ts
...
export class MockLivestockHarvestResultRepository
  implements HarvestResultRepository
{
  ...
  async getAll(): Promise<HarvestResult[]> {
    if (!this.cache) {
      this.cache = await fetchMockData<HarvestResult[]>(
